...
# Function to extract text from an image using PyTesseract OCR and return text before a symbol
def extract_text_from_image(image, symbol):
...
roi_x = int(100 / width * 100)  # Convert to percentage
roi_y = int(840 / height * 100)  # Convert to percentage
roi_width = int(270 / width * 100)  # Convert to percentage
...
initial_text = extract_text_from_image(initial_roi, symbol="«")
final_text = extract_text_from_image(final_roi, symbol="«")
...
if initial_username == final_username:
    ...
    print("User commented on Youtube.")
